...
export default function Mandadito({ params }) {
      ...
      try {
        ...
        setMandadito({
          ...
          offer: {
            ...
            hour: data.offer.createdAt.split("T")[1].split(".")[0],
          },
  ...
  const fetchMandadito = async () => {
    try {
      const res = await fetch(`${API_URL}/api/mandadito/${id}`, {
        method: "GET",
        headers: {
  ...
  if (error) {
    return <Paragraph text={'Hubo un error al traer la información, lo sentimos intenta mas tarde'} color={theme.colors.primary} />;
  }
