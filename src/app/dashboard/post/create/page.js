...
export default function MandaditoNuevo() {
  ...
  const handleSubmit = async (e) => {
    ...
    const locationError = validateLocations(
      formData.IdPickupLocation,
      formData.IdDeliveryLocation
    );

    if (emptyError || titleError || descriptionError || locationError) {
      setErrorMessage(
        emptyError || titleError || descriptionError || locationError
      );
      setIsDisabled(false);
      return;
    }
    ...
    try {
      ...
      setErrorMessage(
        error.message || "Error al crear un post. Intentalo de nuevo"
      );
    }
  };
  ...
  return (
    <PageContainer>
      ...
      <Paragraph
        text={"Completa los datos de tu mandadito"}
      ...
      <form onSubmit={(e) => handleSubmit(e)}>
        ...
        >
          <LocationSelect
            ...
            name={"IdPickupLocation"}
          />
          <LocationSelect
            text={"¿Donde es tu mandadito?"}
            optionList={locations}
            onChange={handleChange}
          ...
          <Input
            label={"Título"}
            placeholder={"e.g. papas fritas del CC"}
          ...
          <Textarea
            label={"Descripción"}
            placeholder={
              "e.g. Necesito que alguien me compre unas papas frita del CC y me las traiga lo antes posible"
            }
          ...
          <Button
            disabled={isDisabled}
            text={isDisabled ? "Posteando..." : "Postear"}
            width={"90%"}
            paddingy={"15px"}
            className={"mt-5"}
            type="submit"
      ...
      <StatusPopUp
        isOpen={isOpenPopup}
        onClose={() => onCloseModal(responsePopup.isSuccess)}
